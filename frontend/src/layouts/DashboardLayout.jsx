...
import { LogOut, User, LayoutDashboard, CalendarCheck, Pill, History, MessageSquareText, Settings, ClipboardList, PhilippinePeso, Bell, ChevronRight, Building2, HelpCircle } from "lucide-react";
...
function PatientSidebarNav({ patientNavItems, location }) {
...
function PatientSettingsLink() {
  ...
  return (
    <SidebarMenuItem>
      ...
      >
        <Link to="/patient/settings" onClick={() => { if (isMobile) setOpenMobile(false); }}>
...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    // 15-second polling interval for real-time consistency
      ...
      <Sidebar collapsible="icon" className="border-r border-slate-200 bg-white">
        ...
        <SidebarContent className="px-2 pt-4">
          <SidebarGroup>
            ...
            <SidebarGroupContent>
              <PatientSidebarNav patientNavItems={patientNavItems} location={location} />
            </SidebarGroupContent>
          </SidebarGroup>
        </SidebarContent>
        ...
        <header className="sticky top-0 z-30 flex h-14 sm:h-16 shrink-0 items-center gap-2 sm:gap-4 border-b border-slate-200 bg-white px-3 sm:px-6 backdrop-blur-md justify-between shadow-2xs">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <Button
              ...
              className="flex items-center gap-1.5 h-8 sm:h-9 px-2.5 sm:px-3 text-xs font-semibold text-slate-700 hover:text-slate-950 border-slate-200 bg-slate-50/80 hover:bg-slate-100"
